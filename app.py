...
def predict_image(model, image_array):
    ...
    top2_conf = sorted_probs[-2]

    margin = top1_conf - top2_conf

    # ✅ FIXED: Lowered thresholds — CIFAR-10 models rarely exceed 80% on real photos.
    # Reject only when the model is genuinely uncertain:
    #   - confidence below 45% (model has no clear winner)
    #   - OR margin below 10% (two classes are neck-and-neck)
    if top1_conf < 0.45 or margin < 0.10:
        return "Others", top1_conf, predictions

    return top1_idx, top1_conf, predictions

def display_prediction_results(predicted_class_idx, confidence, all_predictions, image):

    if predicted_class_idx == "Others":
